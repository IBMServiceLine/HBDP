...
public class CloudantClientManager {
	...
	private static Logger LOGGER = Logger.getLogger(CloudantClientManager.class
			.getName());

	private static CloudantClient cloudant = null;
	private static Database db = null;

	private static String databaseName = "body-measurement";
	...
	private static CloudantClient createClient() {
		...
		if (VCAP_SERVICES != null) {
			...
			obj = (JsonObject) parser.parse(VCAP_SERVICES);
		} else {

			try {
				obj = (JsonObject) parser.parse(new FileReader(
						"D:/Projects/HBDP/src/main/resources/VCAP_SERVICES.json"));
			} catch (FileNotFoundException e) {
				throw new RuntimeException(
						"Could not read VCAP_SERVICES env variables");
			}
		}
	...
	public static Database getDB() {
		LOGGER.info("called getDB ==> " + databaseName);
		if (cloudant == null) {
			initClient();
		}
		...
		if (db == null) {
			try {
				...
				LOGGER.info("got DB ==> " + databaseName);
			} catch (Exception e) {
